  ...
  context "GET /api/user" do
    context "when it didn't find a user" do
      it "returns not found status code" do
        get "/api/user", headers: { "Authorization" => "Bearer #{ Authentication::Token.encode(user_id: 0)}" }
    ...
    context "when it finds a user" do
      ...
      it "should return ok status code" do
        get "/api/user", headers: { "Authorization" => "Bearer #{ Authentication::Token.encode(user_id: user.id)}" }
